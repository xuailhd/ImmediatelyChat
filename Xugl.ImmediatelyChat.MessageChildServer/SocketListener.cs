...
namespace Xugl.ImmediatelyChat.MessageChildServer
{
    ...
    internal class SocketListener:Xugl.ImmediatelyChat.SocketEngine.AsyncSocketListener
    {
        ...
        protected override string HandlerRecivedMessage(string inputMessage)
        {
    ...
    //                string returndata = token.GetReturnData(data);

    //                int bytecount = Encoding.UTF8.GetBytes(token.GetReturnData(data), 0, returndata.Length, e.Buffer, 0);
